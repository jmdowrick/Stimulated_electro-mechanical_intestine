...
Ca_func = dolfinx.fem.Function(V_m_space)
Ca_func.x.array[:n_nodes] = states[:, ca_idx]
Ca_func.x.scatter_forward()
...
diameter_out = dolfinx.fem.Function(V_m_space, name="diameter")

if comm.rank == 0:
    shutil.rmtree(output_path, ignore_errors=True)
comm.Barrier()

vtx = dolfinx.io.VTXWriter(comm, output_path, [V_m_out, V_e_out, diameter_out], engine="BP4")
...
while t < T:
    ...
    # Mechanics
    Ca = states[:, ca_idx]
    T_active = Ca_max * T0 * (1.0 + beta * (lam - 1.0)) * (Ca**h / (Ca50**h + Ca**h))
    radial_strain = T_active / (T_active + k_passive)

    diameter = D0 * (1.0 - radial_strain)

    diameter_function.x.array[:n_nodes] = diameter
    diameter_function.x.scatter_forward()
    ...
    if i % 100 == 0:
        ...
        if comm.rank == 0:
            ...
            print(f"T_active={max(T_active):.2f} ")

        V_m_out.interpolate(w_h.sub(0))
        V_e_out.interpolate(w_h.sub(1))
        diameter_out.x.array[:] = diameter_function.x.array

        V_m_out.x.scatter_forward()
        V_e_out.x.scatter_forward()
        diameter_out.x.scatter_forward()
        vtx.write(t)
